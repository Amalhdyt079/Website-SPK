...
try:
    ...
    if connection.is_connected():
        ...
        for row in rows_criterias:
            ...
            w[row[0]] = row[3]  # Asumsi weight ada di index 3
        ...
        for row in rows_evalutions:
            ...
            try:
                ...
                print(f"Warning: Nilai untuk alternatif {row[0]} dan kriteria {row[1]} tidak valid. Melewatkan baris ini.")
                continue
        ...
        for i, x_i in X.items():
            print(f"Alternatif ID: {i}")
            R[i] = {}
            for j, x_ij in x_i.items():
                print(f"  Kriteria ID: {j}, Nilai: {x_ij}")
                if j in kriteria:  # Pastikan kriteria[j] ada
                    if kriteria[j][1] == 'cost':
                        R[i][j] = min_j[j] / x_ij  # Normalisasi untuk cost (biaya)
                    else:
                        R[i][j] = x_ij / max_j[j]   # Normalisasi untuk benefit (manfaat)
                else:
                    print(f"Warning: Kriteria ID {j} tidak ditemukan dalam dictionary 'kriteria'.")
